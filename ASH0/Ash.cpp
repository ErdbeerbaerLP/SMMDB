...
{
    // Copy the input data into a std::vector (this is what your C++ function expects)
    std::vector<uint8_t> in(input, input + input_size);

    // Call the C++ decompress function
    std::vector<uint8_t> out = Ash::decompress(in);

    // Allocate memory for the decompressed output.
    // (The caller is responsible for freeing this memory with ash_free.)
    uint8_t* result = new uint8_t[out.size()];
    std::copy(out.begin(), out.end(), result);

    // Set the output size if the pointer is provided.
    if (output_size) {
